...
name_sample = list(['qso','sfg'])
inform_path = "/home/kiril/github/AGN_article_final_data/inform"
...
for name_path in name_sample:
    output_path_mod_one = f"{inform_path}/model/mod_{name_path}_normal_1"
    output_path_weight_one = f"{inform_path}/model/weight_{name_path}_normal_1"
    output_path_mod_dark = f"{inform_path}/model/mod_{name_path}_dark_1"
    output_path_weight_dark = f"{inform_path}/model/weight_{name_path}_dark_1"
    ...
    for name in os.listdir(slice_path):
        ...
        if(not name=="file_"):
            ...
            local_ML(f"{output_path_predict}_{name}_{name_path}_normal.csv",data,train[features],batch_size,output_path_mod_one,output_path_weight_one,optimizer,loss)
            local_ML(f"{output_path_predict}_{name}_{name_path}_dark.csv",data,train[features],batch_size,output_path_mod_dark,output_path_weight_dark,optimizer,loss)
